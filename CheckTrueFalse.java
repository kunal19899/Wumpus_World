...
public class CheckTrueFalse {
	...
	public static int entail = 0, non_entail = 0;
	public static HashMap<String, Integer> symbols = new HashMap<String, Integer>();
	public static boolean flag = false, entails;
	...
	public static void main(String[] args) {
			...
			while(  ( buffer = inputStream.readLine() ) != null) 
                        {
                                ...
                                {
					LogicalExpression subExpression = readExpression( buffer );
					String expression = getSymbol(subExpression, true);
					knowledge_base.setSubexpression( subExpression );
                                } 
                                else 
                                {
		...
		TT_Check_All(knowledge_base, statement, 0, 1);
		TT_Check_All(knowledge_base, statement, 0, 0);
		
		if (entail == 0 && non_entail == 0){
			result = "Both True and False";
		}
		else if (entail > 0 && non_entail == 0){
	...
	private static void TT_Check_All(LogicalExpression knowledge_base, LogicalExpression statement, int model, int symbol){
		int i;
		boolean flag = false;

		if (model <= 63){
			for (i = model; i < 64; i++){
				if (symbols.get(hash[i][0]) == 2){
					flag = true;
					break;
				}
			}
			...
			if (flag == true){
				symbols.put(hash[i][0], symbol);
				TT_Check_All(knowledge_base, statement, i + 1, 1);
                TT_Check_All(knowledge_base, statement, i + 1, 0);
                symbols.put(hash[i][0], 2);
			}
		}

		if (model == 64 && symbol == 1){
			boolean knowledge_base_val = find_true(knowledge_base), alpha = find_true(statement);
			if (knowledge_base_val){
				if (alpha){
					entail++;
				}
				else{
	...
	static boolean find_true(LogicalExpression expression){
		int val;
		Enumeration e = expression.getSubexpressions().elements();
		if (!(expression.getUniqueSymbol() == null)){
			val = symbols.get(expression.getUniqueSymbol());
			if (val == 1){
				return true;
			}
			else{
		...
		if (expression.getConnective().equalsIgnoreCase("not")) {
            boolean flag = find_true( (LogicalExpression) e.nextElement());
            return !flag;
        }

		if (expression.getConnective().equalsIgnoreCase("and")) {
			boolean goal = true;
			int count = expression.getSubexpressions().size();
			while (count > 0) {
                boolean flag = find_true((LogicalExpression) e.nextElement());
                goal = goal && flag;
                count--;
		...
		if (expression.getConnective().equalsIgnoreCase("iff")) {
            boolean flag1 = find_true((LogicalExpression) e.nextElement());
            boolean flag2 = find_true((LogicalExpression) e.nextElement());
		...
		if (expression.getConnective().equalsIgnoreCase("or")) {
			boolean goal = false;
            int count = expression.getSubexpressions().size();
            while (count > 0) {
                boolean flag = find_true((LogicalExpression) e.nextElement());
		...
		if (expression.getConnective().equalsIgnoreCase("if")) {
			if (find_true((LogicalExpression) e.nextElement()) && !find_true((LogicalExpression) e.nextElement())) {
                return false;
            } else {
                return true;
            }
	...
	static String getSymbol(LogicalExpression expression, boolean x){
		String a, b = "", connect;
		int val;
		connect = expression.getConnective();

		if (!((a = expression.getUniqueSymbol()) == null)){
			val = symbols.get(a);
			if (val == 2 && x == true){
				symbols.put(a, 1);
			}
		...
		if (connect.equalsIgnoreCase("not")){
			LogicalExpression expression2;
			Enumeration e = expression.getSubexpressions().elements();
			expression2 = (LogicalExpression) e.nextElement();
			System.out.println(expression2);
			b = getSymbol(expression2, false);
			val = symbols.get(b);
			if (val == 2){
				symbols.put(b, 0);
			}
